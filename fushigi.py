...
y_min = -y_max
#
in_gear = gear_profile.Gear(42, module, 0.381010402575401, fig='a')      # 固定
sun_gear = gear_profile.Gear(15, module, -0.307006017901236)             # 平歯車
planet_gear = gear_profile.Gear(15, module, -0.307006017901236)          # 平歯車
in_gear2 = gear_profile.Gear(45, module, -0.921018053703708, fig='a')    # 出力
# 平歯車と平歯車の中心距離
# 平歯車1の歯数、転位係数　z1 s1
# 平歯車2の歯数、転位係数　z2 s2
# モジュール　m
...
circle_max = in_gear.get_hazoko_diameter() + 4.0
circle_cos = [np.nan]
circle_sin = [np.nan]
...
def data_gen():
    for cnt in itertools.count():
        if cnt > 200:
            ...
            ang_inner = 0
            ang_sun = ang_v*cnt*(1 + sun_gear.hasuu0/in_gear.hasuu0)
            ang_planet = -ang_v*cnt*sun_gear.hasuu0/planet_gear.hasuu0
            ang_carrier = ang_v*cnt*sun_gear.hasuu0/in_gear.hasuu0
