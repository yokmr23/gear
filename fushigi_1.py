...
def inner_fig(hazoko_r):
    ...
    circle_cos2 = np.concatenate((circle_cos2, [circle_max2*.95]))
    circle_sin2 = np.concatenate((circle_sin2, [0.0]))
...
txt = ax.text(x_min+0.2, y_max-1., f'Sun gear(tomato):Input:{ang_sun}deg\n'
...
def run(data):
    ...
    txt.set(text=f'Sun gear(tomato):Input:{v_sun*180/np.pi:.8f}deg\n'
