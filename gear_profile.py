...
class Gear:
    ...
    def get_width_teeth_arc(self) -> float:
        """
        歯幅の基準円の円弧の長さ
        """
        return self.mo0 * np.pi + 4 * self.mo0 * self.shift

    def get_width_teeth_ang(self) -> float:
        """
        歯幅の基準円の円弧角度
        """
        return self.get_width_teeth_arc() / self.hasuu0 / self.mo0 * 2

    def get_width_kijunen_kisoen(self) -> float:
        """
    ...
    # def get_width_hazoko_ang(self) -> float:
    #    """
    #    歯底幅角度
    #    """
    ...
    def hazoko_center_kijun(self) -> float:
        """
    ...
    def kijun_hazoko_edge(self) -> float:
        """
        ...
        if self.d0 < self.hazoko:
            a0 -= np.tan(np.arccos(self.d0 / self.hazoko)) - np.arccos(
                self.d0 / self.hazoko
            )
        return a0

    def hazoko_edge_center(self) -> list:
        """
        歯底エッジと歯底中央
    ...
    def get_func1(self, a: float) -> float:
        ang = np.arccos(a)
        return np.tan(ang) - ang

    def gear_hazoko_part(self) -> Any:
        """
        歯底部作成
        ...
        if self.d0 > self.hazoko:
            radius = np.array([self.hazoko / 2.0, self.hazoko / 2.0])
    ...
    def involute_curve(self) -> Any:
        """
        片側インボリュート作成
        """
        if self.hazoko > self.d0:
            theta_sa = self.get_func1(self.d0 / self.hazoko)
            start_radius = self.hazoko / 2.0

        else:
            theta_sa = 0.0
            start_radius = self.d0 / 2.0
        end_radius = self.hasaki / 2.0
        radius = np.linspace(start_radius, end_radius, N1)  # 長さ
        theta = self.get_func1(self.d0 / 2.0 / radius) - theta_sa
        code = np.full(N1, 2)
        return [radius, theta, code]

    def gear_hasaki_part(self) -> Any:
        """
        片側歯先部作成
        """
        # code = np.array([])
        theta = self.hasaki_edge_center()
        radius = np.full(N2, self.hasaki / 2.0)
        theta = np.linspace(theta[0], theta[1], N2)
        code = np.full(N2, 2)
        return [radius[1:], theta[1:], code[1:]]

    def gear_half(self) -> Any:
        """
        一枚の歯車を作成
        戻り値:x座標、y座標、コード
        """
    ...
    def generate_haguruma(self) -> None:
