...
def inner_fig(hazoko_r):
    ...
    circle_cos2 = np.append(circle_cos2, [circle_max2*.95])
    circle_sin2 = np.append(circle_sin2, [0.0])
...
# 初期表示
fig = plt.figure(figsize=(6.4, 6.4), facecolor=(.5, .5, .6))
    ...
    for cnt in itertools.count():
        if cnt > 2000:
            ...
            if flag:
                ...
                ang_planet += ang_v*abs(sp_car-sp_sun) * \
                    sun_gear.hasuu0/planet_gear.hasuu0
                ang_inner += ang_v*sp_car * \
                    (sun_gear.hasuu0 / in_gear2.hasuu0 + 1)\
                    + ang_v*sp_sun*sun_gear.hasuu0/in_gear2.hasuu0
            else:
                ang_carrier += ang_v*sp_car
                ang_planet -= ang_v * sp_car*in_gear2.hasuu0/planet_gear.hasuu0
                ang_sun += ang_v*sp_car * \
                    (in_gear2.hasuu0+sun_gear.hasuu0)/sun_gear.hasuu0
